# Set up the structure of the metadata table
metadata<-data.frame(matrix(ncol = 13, nrow = 0))

## without the short series name first
x <- c('db_table_name','full_series_name',
...
#------------------------------------------------------------------------------------
# Code used to get the metadata for EIA datasets
library(eia)
## read in my eia api key
source(here("etl", "fetch_from_eia_api.R"))
...
## Create an empty list to store the metadata tables in the structure we want
fit_meta<-vector("list", length(series_id_list))

## create lists to store other info that is not in the metadata we already have
...
for (i in 1:length(series_id_list)){
  fit_meta[[i]]<-data.frame(db_table_name = db_table_names[[i]],
