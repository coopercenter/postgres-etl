...
data_series <- get_FRED_series(fredKey,"VANGSP")

#Clean up data, gets rid of first two columns.
data_series <- select(data_series, c(3:4))
...
db_table_names <- lapply("VANGSP",get_name)

#Upload data into database.
dbWriteTable(db, db_table_names[[1]], value = data_series, append = FALSE, overwrite = TRUE, row.names = FALSE)
