pjm_today <- function() {
  ...
  gen_data_today[, date_time := parse_date_time(datetime_beginning_ept, "Ymd HMS", tz =
                                                  "America/New_York")]   #+hours(5)
  gen_data_today[, `:=`(
    date = date(date_time),
    hour = hour(date_time),
    year = year(date_time),
  ...
  #
  base = gen_data_today[hour %in% c(0, 1, 2, 3) &
                          fuel_type == 'Solar', max(mw)]
  if (length(base) == 0)
    return(NULL)
...
pjm_generation_by_fuel <-
  function(gen_data, file, last_date_retreived) {
    library(httr)
    library(lubridate)
    ...
    if (yesterday - last_date_retrieved > 1) {
      retrieve_date_range = paste0(last_date_retrieved + 1,
                                   "T00:00:00 to ",
                                   yesterday,
                                   "T23:00:00")
    ...
    gen_data_part[, date_time := parse_date_time(datetime_beginning_ept, "Ymd HMS", tz =
                                                   "America/New_York")]   #+hours(5)
    gen_data_part[, `:=`(
      date = date(date_time),
      hour = hour(date_time),
    ...
    for (i in seq_along(unique(gen_data_part$date))) {
          ...
          if (abs(early_morning_solar - late_evening_solar) < 0.2) {
            ...
            gen_data_part[date == this_date & fuel_type == "Solar",
                          mw := ifelse(
                            ...
                            ifelse(
                              mw_solar %in% c(
                                late_evening_solar - 0.1,
                                late_evening_solar,
                                late_evening_solar + 0.1
                              ),
                              mw_solar - late_evening_solar,
                              mw_solar - mean(c(
                                early_morning_solar, late_evening_solar
                              ))
                            )
                          )]
            gen_data_part[mw < 0 & fuel_type == "Solar", mw := 0.0]
      ...
      print(gen_data_part[date == unique(gen_data_part$date)[i] &
                            fuel_type == "Solar", .(daily_total = sum(mw))])
    }
    #
    gen_data_part[, solar_generation_mw := NA]
    gen_data_all = rbindlist(list(gen_data, gen_data_part),
                             use.names = TRUE,
                             fill = FALSE)
...
solar_hourly <- function() {
  ...
  retrieve_date_range = paste0(date(last_date_recorded + 1),
                               "T00:00:00 to ",
                               date(now()) - 1,
                               "T23:00:00")
  print(retrieve_date_range)
  r <- GET(
    "https://api.pjm.com/api/v1/solar_gen",
    add_headers("Ocp-Apim-Subscription-Key" = "625845c6fabc4639ab91428486d8d2e2"),
    query = list(
      rowCount = "50000",
      startRow = "1",
  ...
  bad_dates = solar_data_new[hour == 12 &
                               area == "RTO" & solar_generation_mw <= 0, date]
  print(bad_dates)
  solar_gen_data = rbindlist(list(solar_data_local, solar_data_new[date %!in%
                                                                     bad_dates]))
