...
con_per_cap<-dbGetQuery(db,'SELECT * from energy_consumption_per_capita_va')
...
r1<- data.frame(db_table_name = "energy_consumption_per_capita_va",
                ...
                series_id=I(list(c('SEDS.TETCB.VA.A','VAPOP'))),json=NA,notes=NA, mandate=0, forecast=0, corresponding_data=I(list(c('eia_seds_tetcb_va_a','residential_population_va'))), 
                R_script='deriving_energy_consumption_per_capita')
